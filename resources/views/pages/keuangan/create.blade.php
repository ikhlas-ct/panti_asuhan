...
<style>
    .section-divider {
        ...
        font-weight:600; font-size:.9rem; color:#1a73e8; margin-bottom:1rem;
    }
    ...
    .ph-card.create-page::before { background:#1a73e8; }
    ...
    .ph-icon.create { background:#e8f0fe; color:#1a73e8; }
    ...
    .donasi-info-box {
        ...
        padding:12px 16px; font-size:.83rem; color:#1d4ed8;
        display:none;
    }
                        ...
                        <div class="card-body">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-12">
                                    ...
                                    <div class="d-flex gap-2">
                                        <button type="button" class="jenis-btn {{ old('jenis', 'pemasukan') === 'pemasukan' ? 'active-pemasukan' : '' }}"
                                            id="btn-pemasukan" onclick="setJenis('pemasukan')">
                                            <i class="fas fa-arrow-circle-down me-1"></i> Pemasukan
                                        </button>
                                        <button type="button" class="jenis-btn {{ old('jenis') === 'pengeluaran' ? 'active-pengeluaran' : '' }}"
                                            id="btn-pengeluaran" onclick="setJenis('pengeluaran')">
                                            <i class="fas fa-arrow-circle-up me-1"></i> Pengeluaran
                                        </button>
                                    </div>
                                    <input type="hidden" name="jenis" id="jenis" value="{{ old('jenis', 'pemasukan') }}">
                                    @error('jenis')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                                </div>

                                {{-- Link dari Donasi (khusus pemasukan) --}}
                                <div id="section-donasi" class="col-12">
                                    <label class="form-label">Pemasukan dari Donasi
                                        <span class="text-muted fw-normal">(opsional — isi jika transaksi ini berasal dari donasi)</span>
                                    </label>
                                    <select name="donasi_id" id="donasi_id"
                                        ...
                                        @foreach($donasiList as $d)
                                            <option value="{{ $d->id }}"
                                                ...
                                                data-donatur="{{ $d->donatur->nama ?? '-' }}"
                                                {{ old('donasi_id') == $d->id ? 'selected' : '' }}>
                                                {{ $d->donatur->nama ?? '-' }} —
                                    ...
                                    <div class="donasi-info-box mt-2" id="donasi-info">
                                        <i class="fas fa-info-circle me-1"></i>
                                        Nominal & keterangan akan diisi otomatis dari data donasi terpilih.
                                        Nominal dapat diubah manual jika perlu.
                                    </div>
                                </div>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-body">
                            ...
                            <p class="text-muted small mb-3">Upload foto bukti transfer / kwitansi (opsional).</p>
...
function setJenis(jenis) {
    ...
    // Section donasi hanya tampil untuk pemasukan
...
// Init
setJenis(document.getElementById('jenis').value || 'pemasukan');
...
@unless($isAdminPanti)
// ── Admin dinsos: reload donasi saat panti berubah ─────────
