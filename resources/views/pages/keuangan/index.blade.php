    ...
    <div class="container">
        ...
        <div class="ph-card index-page">
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('keuangan.laporan.form') }}" class="btn btn-outline-primary btn-sm">
            ...
            <div class="row g-3 mb-4">
                ...
                <div class="col-md-4">
                    <div class="stat-card {{ $saldo >= 0 ? 'blue' : 'red' }}">
                        <div class="d-flex align-items-center gap-3">
                            <div class="stat-icon {{ $saldo >= 0 ? 'blue' : 'red' }}"><i class="fas fa-balance-scale"></i>
                            </div>
                            <div>
                    ...
                    <form method="GET" action="{{ route('keuangan.index') }}">
                        <div class="row g-2 align-items-end">
                            ...
                            @unless ($isAdminPanti)
                                <div class="col-12 col-md-3">
                                    <select name="panti_asuhan_id" class="form-select">
                                        ...
                                        @foreach ($pantis as $p)
                                            <option value="{{ $p->id }}"
                                                {{ request('panti_asuhan_id') == $p->id ? 'selected' : '' }}>
                                                {{ $p->nama_panti }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-6 col-md-2">
                                <select name="tahun" class="form-select">
                                    ...
                                    @foreach (range(date('Y'), date('Y') - 5) as $y)
                                        <option value="{{ $y }}"
                                            {{ request('tahun') == $y ? 'selected' : '' }}>{{ $y }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-auto">
                                ...
                                <a href="{{ route('keuangan.index') }}" class="btn btn-outline-secondary btn-sm ms-1"
                                    title="Reset">
                            ...
                            <tbody>
                                @forelse($keuangans as $i => $item)
                                    <tr>
                                        ...
                                        @unless ($isAdminPanti)
                                            <td>
                                                <div style="font-size:.85rem;color:#1e293b;">
                                                    {{ $item->pantiAsuhan->nama_panti ?? '-' }}</div>
                                            </td>
                                        @endunless
                                        ...
                                        <td>
                                            <div style="max-width:200px;font-size:.83rem;" class="text-truncate"
                                        ...
                                        <td class="nominal-{{ $item->jenis }} text-end">
                                            {{ $item->jenis === 'pengeluaran' ? '- ' : '+ ' }}Rp
                                            {{ number_format($item->nominal, 0, ',', '.') }}
                                        </td>

                                        <td>
                                            @if ($item->donasi_id)
                                                <span class="badge badge-donasi">
                                        ...
                                        <td>
                                            <div class="d-flex gap-1">
                                                ...
                                                <button class="btn btn-action btn-hapus" data-id="{{ $item->id }}"
                                                    data-nominal="Rp {{ number_format($item->nominal, 0, ',', '.') }}"
                                                    title="Hapus">
                                                    <i class="fas fa-trash-alt"></i>
                                                </button>
                                                <form id="form-hapus-{{ $item->id }}"
                                                    action="{{ route('keuangan.destroy', $item) }}" method="POST"
                                                    class="d-none">
                                                    @csrf @method('DELETE')
                                                </form>
                                            </div>
                ...
                @if ($keuangans->hasPages())
                    <div class="card-footer d-flex justify-content-between align-items-center flex-wrap gap-2">
                        <small class="text-muted">
                            Menampilkan
                            <strong>{{ $keuangans->firstItem() }}</strong>–<strong>{{ $keuangans->lastItem() }}</strong>
