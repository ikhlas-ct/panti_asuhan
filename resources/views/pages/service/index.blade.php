                            ...
                            <tbody>
                                @forelse ($services as $index => $service)
                                    <tr>
                                        ...
                                        @if ($type === 'transportasi')
                                            ...
                                            <td>
                                                @if ($service->gambar && file_exists(public_path($service->gambar)))
                                                    <img src="{{ asset($service->gambar) }}" alt="{{ $service->title }}"
                                        ...
                                        @endif
                                        <td class="font-weight-bold text-center">{{ $service->order ?? '-' }}</td>
                                        @if (in_array($type, ['tema', 'layanan', 'transportasi']))
                                            <td class="text-center">
                                                <span class="badge badge-primary">{{ $service->steps_count ?? 0 }}
                                                    step</span>
                                            </td>
                                        @endif
                                        <td class="text-center">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="{{ in_array($type, ['tema', 'layanan']) ? 7 : ($type === 'transportasi' ? 8 : ($type === 'informasi' ? 5 : 6)) }}"
