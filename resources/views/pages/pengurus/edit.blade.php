...
@section('title', 'Edit Pengurus - ' . $pengurus->nama)
...
<div class="container">
    ...
    <div class="ph-card">
        <div class="ph-left">
            ...
            <div>
                <h5 class="ph-title">Edit Pengurus</h5>
                <ol class="ph-breadcrumb">
                    <li><a href="{{ route('dashboard') }}">Dashboard</a></li>
                    <li><a href="{{ route('pengurus.index') }}">Pengurus</a></li>
                    <li><a href="{{ route('pengurus.show', $pengurus) }}">{{ $pengurus->nama }}</a></li>
                    <li><span class="bc-active">Edit</span></li>
                </ol>
            </div>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('pengurus.show', $pengurus) }}" class="btn btn-primary btn-sm">
    ...
    <div class="page-inner">
        <form action="{{ route('pengurus.update', $pengurus) }}" method="POST" enctype="multipart/form-data" novalidate>
            @csrf
            @method('PUT')
            <div class="row g-4">
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-body">
                            ...
                            <div class="row g-3">
                                    ...
                                    <select name="panti_asuhan_id" class="form-select @error('panti_asuhan_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($pantis as $panti)
                                            <option value="{{ $panti->id }}"
                                                {{ old('panti_asuhan_id', $pengurus->panti_asuhan_id) == $panti->id ? 'selected' : '' }}>
                                                {{ $panti->nama_panti }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                @else
                                    <input type="hidden" name="panti_asuhan_id" value="{{ $pengurus->panti_asuhan_id }}">
                                @endif
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select name="pendidikan_terakhir" class="form-select @error('pendidikan_terakhir') is-invalid @enderror">
                                        ...
                                        @foreach(['SD','SMP','SMA/SMK','D1','D2','D3','S1','S2','S3'] as $p)
                                            <option value="{{ $p }}"
                                                {{ old('pendidikan_terakhir', $pengurus->pendidikan_terakhir) === $p ? 'selected' : '' }}>
                                                {{ $p }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="alamat" rows="2"
                                        class="form-control @error('alamat') is-invalid @enderror">{{ old('alamat', $pengurus->alamat) }}</textarea>
                                    @error('alamat')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-body text-center">
                            ...
                            <div class="foto-preview-wrap mx-auto mb-3"
                                ...
                                <img id="foto-preview"
                                    src="{{ ($pengurus->foto && file_exists(storage_path('app/public/' . $pengurus->foto)))
                                        ? asset('storage/' . $pengurus->foto)
                                        : 'https://ui-avatars.com/api/?name=' . urlencode($pengurus->nama) . '&background=f5f3ff&color=7c3aed&size=120' }}"
                                    alt="{{ $pengurus->nama }}">
                            </div>
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-body">
                            ...
                            <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                ...
                                <option value="nonaktif" {{ old('status', $pengurus->status) === 'nonaktif' ? 'selected' : '' }}>Non-aktif</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-body">
                            ...
                            @php
                                $defaultMode = $pengurus->user_id ? 'existing' : 'none';
                            ...
                            <div class="d-flex flex-column gap-2 mb-3">
                                ...
                                <label class="akun-radio-opt {{ $currentMode === 'existing' ? 'active' : '' }}"
                                       style="{{ $pengurus->user_id ? '' : 'display:none' }}" id="opt-existing">
                            ...
                            <div id="box-existing" class="akun-box mb-0"
                                ...
                                <div class="d-flex align-items-center gap-2 mb-3">
                                    <div style="width:36px;height:36px;border-radius:50%;background:#e8f0fe;display:flex;align-items:center;justify-content:center;color:#1a73e8;flex-shrink:0;">
                                        <i class="fas fa-user"></i>
                                    </div>
                                    <div>
                                        <div style="font-size:.85rem;font-weight:600;color:#1e293b;">{{ $pengurus->user->name }}</div>
                                        <div style="font-size:.75rem;color:#64748b;">{{ $pengurus->user->email }}</div>
                                    </div>
                                </div>
                                <div class="row g-2">
                                    <div class="col-12">
                                        <label class="form-label" style="font-size:.8rem;">
                                            Password Baru <span class="optional-hint">(kosongkan jika tidak ingin ganti)</span>
                                        </label>
                            ...
                            <div id="box-baru" class="akun-box mb-0"
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="input-group">
                                            <input type="password" name="akun_password" id="pw3"
                                                class="form-control @error('akun_password') is-invalid @enderror"
                                                placeholder="Min. 8 karakter">
                                            <button type="button" class="btn btn-outline-secondary" onclick="togglePw('pw3',this)">
                                                <i class="fas fa-eye"></i>
                                            </button>
                                        </div>
                                        @error('akun_password')
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <button type="button" class="btn btn-outline-secondary" onclick="togglePw('pw4',this)">
                                                <i class="fas fa-eye"></i>
                                            </button>
                                        </div>
                                    </div>
                                ...
                                <div class="mt-2 p-2 rounded" style="background:#f0fdf4;font-size:.75rem;color:#15803d;">
                                    ...
                                    Akun baru dibuat dengan role <strong>admin_panti</strong> dan status <strong>aktif</strong>.
                                </div>
                            </div>
                    ...
                    <div class="d-grid gap-2">
                        ...
                        <a href="{{ route('pengurus.show', $pengurus) }}" class="btn btn-outline-secondary">Batal</a>
                    </div>
...
<script>
    ...
    function applyMode(val) {
        ...
        radioOpts.forEach(opt => {
            ...
            opt.classList.toggle('active', r.checked);
        });
    }
