...
@section('title', 'Edit Donatur - ' . $donatur->nama)
...
<style>
    ...
    .ph-card::before {
        ...
        width: 4px; border-radius: 14px 0 0 14px; background: #e96c1a;
    }
    .ph-left { display: flex; align-items: center; gap: 12px; }
    .ph-icon {
        width: 42px; height: 42px; border-radius: 10px; flex-shrink: 0;
        display: flex; align-items: center; justify-content: center; font-size: 1rem;
        background: #fff4ed; color: #e96c1a;
    }
    ...
    .form-control:focus {
        ...
        box-shadow: 0 0 0 3px rgba(233,108,26,.1);
    }
    ...
    .akun-radio-opt input[type=radio] { accent-color: #e96c1a; }
    .akun-radio-opt.active { border-color: #e96c1a; background: #fff9f5; }
    .akun-radio-opt:hover  { border-color: #fbbf9a; }
    .akun-box {
        border: 1.5px dashed #e2e8f0; border-radius: 12px;
        padding: 16px; background: #fafbfc; transition: border-color .2s;
    }
    .akun-box.active { border-color: #e96c1a; background: #fff9f5; }
</style>
@endsection
...
<div class="container">
    ...
    <div class="ph-card">
        <div class="ph-left">
            ...
            <div>
                <h5 class="ph-title">Edit Donatur</h5>
                <ol class="ph-breadcrumb">
                    <li><a href="{{ route('dashboard') }}">Dashboard</a></li>
                    <li><a href="{{ route('donatur.index') }}">Donatur</a></li>
                    <li><a href="{{ route('donatur.show', $donatur) }}">{{ $donatur->nama }}</a></li>
                    <li><span class="bc-active">Edit</span></li>
                </ol>
            </div>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('donatur.show', $donatur) }}" class="btn btn-primary btn-sm">
    ...
    <div class="page-inner">
        <form action="{{ route('donatur.update', $donatur) }}" method="POST" enctype="multipart/form-data" novalidate>
            @csrf
            @method('PUT')
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select name="jenis_donatur" id="jenis_donatur"
                                        ...
                                        @foreach(['perorangan' => 'Perorangan','organisasi' => 'Organisasi','perusahaan' => 'Perusahaan','pemerintah' => 'Pemerintah'] as $val => $lbl)
                                            <option value="{{ $val }}"
                                                {{ old('jenis_donatur', $donatur->jenis_donatur) === $val ? 'selected' : '' }}>
                                                {{ $lbl }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input type="text" name="no_telp" maxlength="20"
                                        ...
                                        value="{{ old('no_telp', $donatur->no_telp) }}"
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="alamat" rows="2"
                                        ...
                                        placeholder="Alamat lengkap">{{ old('alamat', $donatur->alamat) }}</textarea>
                                    @error('alamat')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="keterangan" rows="2"
                                        ...
                                        placeholder="Catatan tambahan…">{{ old('keterangan', $donatur->keterangan) }}</textarea>
                                    @error('keterangan')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-body text-center">
                            ...
                            <div class="foto-preview-wrap mx-auto mb-3"
                                ...
                                <img id="foto-preview"
                                    src="{{ ($donatur->foto && file_exists(storage_path('app/public/'.$donatur->foto)))
                                        ? asset('storage/'.$donatur->foto)
                                        : 'https://ui-avatars.com/api/?name='.urlencode($donatur->nama).'&background=e0f2fe&color=0891b2&size=120' }}"
                                    alt="{{ $donatur->nama }}">
                            </div>
                            ...
                            <div class="text-muted small mt-2">Kosongkan jika tidak ingin mengubah</div>
                            @error('foto')
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-body">
                            ...
                            <select name="status" id="select-status"
                                ...
                                <option value="aktif"    {{ old('status', $donatur->status) === 'aktif'    ? 'selected' : '' }}>Aktif</option>
                                <option value="nonaktif" {{ old('status', $donatur->status) === 'nonaktif' ? 'selected' : '' }}>Non-aktif</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-body">
                            ...
                            @php
                                $defaultMode = $donatur->user_id ? 'existing' : 'none';
                                $currentMode = old('mode_akun', $defaultMode);
                            @endphp

                            <div class="d-flex flex-column gap-2 mb-3">
                                <label class="akun-radio-opt {{ $currentMode === 'none' ? 'active' : '' }}">
                                ...
                                @if($donatur->user_id)
                                <label class="akun-radio-opt {{ $currentMode === 'existing' ? 'active' : '' }}" id="opt-existing">
                                    <input type="radio" name="mode_akun" value="existing"
                                        {{ $currentMode === 'existing' ? 'checked' : '' }}>
                                    <span><i class="fas fa-key me-1 text-primary"></i> Akun sudah ada — ganti password</span>
                                </label>
                                @endif

                                <label class="akun-radio-opt {{ $currentMode === 'baru' ? 'active' : '' }}">
                                    <input type="radio" name="mode_akun" value="baru"
                                        {{ $currentMode === 'baru' ? 'checked' : '' }}>
                                    <span><i class="fas fa-user-plus me-1 text-success"></i> Buat akun baru</span>
                                </label>
                            </div>

                            {{-- Box: akun existing - ganti password --}}
                            @if($donatur->user)
                            <div id="box-existing" class="akun-box mb-0"
                                 style="{{ $currentMode === 'existing' ? '' : 'display:none' }}">
                                <div class="d-flex align-items-center gap-2 mb-3">
                                    <div style="width:36px;height:36px;border-radius:50%;background:#e0f2fe;display:flex;align-items:center;justify-content:center;color:#0891b2;flex-shrink:0;">
                                        <i class="fas fa-user"></i>
                                    </div>
                                    <div>
                                        <div style="font-size:.85rem;font-weight:600;color:#1e293b;">{{ $donatur->user->username }}</div>
                                        <div style="font-size:.75rem;color:#64748b;">{{ $donatur->user->email }}</div>
                                    </div>
                                </div>
                                <div class="row g-2">
                                    <div class="col-12">
                                        <label class="form-label" style="font-size:.8rem;">
                                            Password Baru
                                            <span class="optional-hint">(kosongkan jika tidak ingin ganti)</span>
                                        </label>
                                    ...
                                    <div class="col-12">
                                        <label class="form-label" style="font-size:.8rem;">Konfirmasi Password Baru</label>
                                        <div class="input-group">
                            ...
                            {{-- Box: buat akun baru --}}
                            <div id="box-baru" class="akun-box mb-0"
                                 style="{{ $currentMode === 'baru' ? '' : 'display:none' }}">
                                <div class="row g-3">
                                    <div class="col-12">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="input-group">
                                            <input type="password" name="akun_password" id="pw3"
                                                class="form-control @error('akun_password') is-invalid @enderror"
                                                placeholder="Min. 8 karakter">
                                            <button type="button" class="btn btn-outline-secondary"
                                                    onclick="togglePw('pw3',this)">
                                                <i class="fas fa-eye"></i>
                                            </button>
                                        </div>
                                        @error('akun_password')
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="input-group">
                                            <input type="password" name="akun_password_confirmation" id="pw4"
                                                class="form-control" placeholder="Ulangi password">
                                            <button type="button" class="btn btn-outline-secondary"
                                                    onclick="togglePw('pw4',this)">
                                                <i class="fas fa-eye"></i>
                                            </button>
                                        </div>
                                    </div>
                                ...
                                <div class="mt-2 p-2 rounded" style="background:#f0fdf4;font-size:.75rem;color:#15803d;">
                                    ...
                                    Akun baru dibuat dengan role <strong>donatur</strong> dan status <strong>aktif</strong>.
                                </div>
                            </div>
                    ...
                    <div class="d-grid gap-2">
                        ...
                        <a href="{{ route('donatur.show', $donatur) }}" class="btn btn-outline-secondary">Batal</a>
                    </div>
...
<script>
    ...
    const boxExisting = document.getElementById('box-existing');
    const boxBaru     = document.getElementById('box-baru');
    const radioOpts   = document.querySelectorAll('.akun-radio-opt');

    function applyMode(val) {
        if (boxExisting) boxExisting.style.display = val === 'existing' ? '' : 'none';
        if (boxBaru)     boxBaru.style.display     = val === 'baru'     ? '' : 'none';
        radioOpts.forEach(o => o.classList.toggle('active', o.querySelector('input').checked));
    }
